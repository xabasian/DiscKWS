...
/************************************************************************
 ...
 Comments:     none.
 ***********************************************************************/
uint Dataset::read(SpeechUtterance &x, StartTimeSequence &y, bool remove_silence)
{
  uint sil_offset = 0;
  
  if (single_file_mode) {
    std::cout << "current file=" << current_file << " " << scores_file_list[0] << std::endl;
    x.read(scores_file_list[0], dists_file_list[0], 
           phonemes_file_list[current_file],single_file_mode);
  }
  else {
    std::cout << "current file=" << current_file << " " << scores_file_list[current_file] << std::endl;
