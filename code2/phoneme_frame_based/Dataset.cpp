...
void InstanceType::read(std::string &filename, bool print_header)
{
  ...
  for (int i=0; i < my_htk.nSamples(); i++) {
    ...
    if ( rc != int(my_htk.num_coefs()) ) {
      cerr << "Error: Cannot read sample number " << i << " from " << filename  << "." << endl;
      exit(-1);
    }
...
void LabelType::set_silence_symbol(std::string &symbol)
{
  ...
  index2phoneme[0] = silence_symbol;
  phoneme2index[silence_symbol] = 0;
}
...
void LabelType::load_phoneme_map(std::string &phonemes_filename)
{
  ...
  if (!silence_symbol_found) {
    std::cerr << "Error: Silence symbol " << silence_symbol << " was not found in the phonemes file." 
    << std::endl;
  }
...
void LabelType::read(std::string &filename)
{
  ...
  while (ifs.good()) {
    ...
    //    cout << start_time << " " << stop_time << " " << stop_time-start_time << " "<< phoneme << endl;
  }
  ifs.close();
  ...
  if (read_labels) {
    ...
    //cout << target_files[i] << endl;
		if (x.num_frames == y.num_frames-1) {
			...
			cerr << "Warning: number of labels (" << y.num_frames << ") does not match number of frames (" 
			<< x.num_frames << ")" << endl;
